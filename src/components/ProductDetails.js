...
const ProductDetails = () => {
  const { id } = useParams();
  const { cartItems, setCartItems, cartCount, setCartCount, handleAddToCart, handleSubtractFromCart } = useContext(CartContext);
  const [product, setProduct] = useState(null);
  const [productCount, setProductCount] = useState(0);
    ...
    <div className="container mx-auto p-4 flex flex-col md:flex-row">
      ...
      <div className="md:w-2/3 md:pl-8">
        ...
        <div className="flex items-center mb-4">
          <label className="mr-2">Quantity selected:</label>
          <div className="flex items-center">
            <button onClick={handleSubtract} className="bg-red-500 text-white px-2 py-1 rounded">-</button>
            <span className="mx-2">{productCount}</span>
